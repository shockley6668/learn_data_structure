...
public:
    ...
    void MakeTree(int n, int wt[], string chars); // 公有函数，被主函数main调用
...
// 构建huffman树
void HuffMan::MakeTree(int n, int wt[], string chars) {
...
int main()
{
    ...
    while(t--)
    {
        ...
        HuffMan myHuff;
        wt=new int[frequences.size()];
        int i=0;
        string encdesingle;
        for(const auto& pair:frequences)
        {
            wt[i]=pair.second;
            encdesingle.push_back(pair.first);
            i++;
        }
        myHuff.MakeTree((int)frequences.size(), wt, encdesingle);
        myHuff.Coding();
        int codeIndex=0;
        for (int j = 1; j <= frequences.size(); j++)
        {
            codeIndex+=myHuff.huffCode[j].size()*wt[j-1];
        }
        cout<<codeIndex<<endl;
        myHuff.Destroy();
    }
